// src/components/PresidentDirectory.tsx
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setPres(data);
        // fetch counts in parallel (simple and fine for your size)
        const entries = await Promise.all(
          data.map(async (p) => {
            try {
              const vs = await api.listVoyages(
                new URLSearchParams({ president_id: String(p.president_id) })
              );
              return [p.president_id, vs.length] as const;
            } catch {
              return [p.president_id, 0] as const;
            }
          })
        );
        if (!alive) return;
        ...
        {rows.map((p) => (
          ...
          >
            <div>
              ...
              {p.party && (
                <div className="text-xs text-gray-500 mt-1">{p.party}</div>
              )}
            </div>
            <div className="text-sm text-gray-700">
              {counts[p.president_id] ?? 0} voyages →
            </div>
          </Link>
        ))}
      </div>
